...
**The outcome-anchored second opinion — an honest, hard result.** Predicting real critical outcomes from triage-time features (acuity excluded) yields **ROC-AUC 0.813, PR-AUC 0.729** — a deliberately *non-trivial* number, the realistic range for this task; any claim above ~0.95 would betray leakage. The model is well-calibrated overall (ECE 0.014→~0 after isotonic) and *within* every language and age subgroup (ECE 0.001–0.015), so it does not quietly mislead for minority-language or elderly patients. Flagging the top-decile-risk patients among the 34,418 low-acuity (L4/L5) cases surfaces **3,646 undertriage candidates** whose actual critical-outcome rate is 11.9% versus 9.3% for their non-flagged peers — a real signal of missed severity within the "safe" stratum (e.g. a patient logged as *"mild chest discomfort," L4,* whom the model scored at 59.5% risk and who was ultimately admitted). The flags are distributed equitably across all protected groups. Finally, **data forensics** confirm the generator's hand: `mean_arterial_pressure`, `pulse_pressure`, and `shock_index` are exact analytic derivations of base vitals (R² ≥ 0.9999), validating the pipeline and reinforcing that this dataset is a constructed, physiology-driven system.

## Limitations & Reproducibility

The dataset is synthetic, and four properties bound our claims: (1) the label is near-deterministic physiology, so acuity accuracy is an upper bound, not a realistic benchmark; (2) the text AUC of 1.0 is an encoding artefact; (3) inter-rater variability is near-zero, unlike real triage; and (4) there is no demographic bias to find — our null audit result is a *property of the data*, not a statement about real EDs. The Pillar D undertriage signal is correspondingly modest (1.3× enrichment) because low-acuity patients are genuinely low-risk *by construction*; in real EDs, where compensated physiology masks severity, the same method would surface a far stronger signal. We treat each limitation as a teaching contrast rather than hiding it.
...
The deliverables that matter are the two reusable tools. **Pillar C's audit toolkit** (`audit_by_group`, `inject_undertriage`, `audit_inter_rater`, plus the literature-contrast builder) runs **without modification** on MIMIC-IV-ED (~425k stays, identical triage schema) or a hospital's own triage logs to (a) detect protected-attribute disparities, (b) quantify them in interpretable NEWS2-residual units, (c) flag individual nurses for targeted re-training, and (d) benchmark against the literature — with a demonstrated detection floor (~0.05 acuity units) finer than any documented disparity. **Pillar D's `triage_second_opinion()`** is the patient-facing counterpart: an independent, calibrated outcome-risk check that raises a soft undertriage alert *as the nurse works*, catching the "mild chest discomfort" that is really an admission — the same decision-support value as the field's interactive-simulator entries, but anchored to real outcomes and auditable for fairness.
